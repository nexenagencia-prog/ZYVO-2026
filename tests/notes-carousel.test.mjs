...
test('shared sidebar preserves hero profile avatar and original menu actions',()=>{const sidebar=fs.readFileSync('app/AppSidebar.tsx','utf8');assert.match(sidebar,/zyvo-profile-avatar/);assert.match(sidebar,/Camera/);assert.match(sidebar,/Calculadora/);assert.match(sidebar,/Anotar/);assert.match(sidebar,/Anotações/)});
