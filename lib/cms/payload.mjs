...
export function validateHomePayload(input){
  ...
  return {
    ...
    nextMeeting:{
      label:validateRequiredText(nextMeeting.label,'Próxima reunião'),
      dateTime:validateRequiredText(nextMeeting.dateTime,'Data da próxima reunião')
    },
    profile:{
      name:validateRequiredText(profile.name,'Nome do perfil'),
      avatarUrl:typeof profile.avatarUrl==='string'?profile.avatarUrl:'',
      planLabel:validateRequiredText(profile.planLabel,'Plano')
    },
    navigation:{
    ...
    carousel:carousel.map((item,index)=>({
      ...
      title:validateRequiredText(item.title,'Título do carrossel'),
      subtitle:validateRequiredText(item.subtitle,'Subtítulo do carrossel'),
    ...
    cards:cards.map((card,index)=>({
      ...
      slug:validateRequiredText(card.slug,'Slug do card'),
      title:validateRequiredText(card.title,'Título do card'),
      description:validateRequiredText(card.description,'Descrição do card'),
      percentage:validatePercentage(card.percentage),
      imageUrl:typeof card.imageUrl==='string'?card.imageUrl:'',
      ctaLabel:validateRequiredText(card.ctaLabel,'CTA do card'),
