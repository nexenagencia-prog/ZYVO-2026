  ...
  {featured&&<section className="recordings-featured-wrap">
   ...
   <Rail className="recordings-featured-rail">
    {recordings.map((item,index)=><article className={`recordings-featured-card ${index===active?'is-active':''}`} key={item.id} style={{backgroundImage:`linear-gradient(180deg,rgba(4,8,12,.03),rgba(4,8,12,.72)),url(${item.thumbnail})`}} onClick={()=>setActive(index)}>
      <div className="featured-copy"><span>GRAVAÇÃO ZYVO</span><h1 style={{fontFamily:item.font,fontSize:`${item.fontSize}px`}}>{item.title}</h1><p>{item.phrase}</p><div className="recordings-stars" aria-label={`Performance ${item.performance} de 5`}>{[1,2,3,4,5].map(n=><Star key={n} className={n<=item.performance?'filled':''}/>)}</div></div>
      ...
      <CardTools item={item} onEdit={()=>setEditing(item)} onThumb={()=>pickThumbnail(item.id)} onDelete={()=>remove(item.id)} onAnalyze={()=>router.push('/skills')}/>
    </article>)}
   </Rail>
